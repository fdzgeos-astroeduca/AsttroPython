# Programa de Buscador de Asteroides - Con todas las funcionalidades restauradas para Streamlit
...
with st.sidebar:
    ...
    if st.button("Abrir subida a Astrometry"):
...
if st.session_state["resultados_df"] is not None:
    ...
    if asteroide_seleccionado:
        ...
        # Pestañas organizadas para todas las funciones originales
        ...
        with tab3:
            st.markdown("### Tabla de Trayectoria Horaria (Próximтельный 13 horas)")
            dt_base = datetime.strptime(f"{fecha_def} {hora_def}", "%Y-%m-%d %H:%M:%S")
            ...
            for t, ra_v, dec_v, alt_v, mag_v in zip(tiempos_h, ra_h, dec_h, alt_h, mag_h):
                ...
                trayectoria_rows.append({
                    "Hora (UT)": t.strftime("%H:%M UT"),
                    "AR (J2000)": ra_str,
                    "Dec (J2000)": dec_str,
                    "Altura": f"{alt_v:.1f}°",
                    "Mag V": f"{mag_v:.2f}"
                })
            st.dataframe(pd.DataFrame(trayectoria_rows), use_container_width=True)
